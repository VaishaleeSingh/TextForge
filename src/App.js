...
function App() {
  const [themeName, setThemeName] = useState(() => {
    return localStorage.getItem('textforge-theme') || 'light';
  });
  const [theme, setTheme] = useState(getTheme(themeName));
  const [alert, setAlert] = useState(null);
  
  useEffect(() => {
    const savedTheme = localStorage.getItem('textforge-theme') || 'light';
  ...
  return (
    <>
      <Router>
        <Navbar title="TextForge" theme={theme} themeName={themeName} changeTheme={changeTheme} themes={themes} />
